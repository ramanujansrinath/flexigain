...
class curvature_saccade_task(Task):
    def __init__(self, dt=10, tau=20, T=1800, N_in=259, N_out=1, N_batch=512, in_noise=0.01, alpha=1):
        # ----------------------------------
        # Define network parameters
        # ----------------------------------
        super().__init__(N_in, N_out, dt, tau, T, N_batch)
        self.in_noise = in_noise
        self.act = np.random.rand(1000,256)
        self.cc = np.random.rand(1000,1)
                
    def generate_trial_params(self, batch, trial):
        # ----------------------------------
        # Define parameters of a trial
        # ----------------------------------
        idx = rng.choice(range(1000))
        params = dict()
        params['s'] = self.cc[idx] # random row that is the value of K
        params['act'] = self.act[idx,:] # same random row with activations for that shape
